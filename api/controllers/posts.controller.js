...
import { resourceModel, references } from "../../config/resource";
...
export const index = async (req, res) => {
	try {
		...
		const posts = await resourceModel["posts"]
			.find({}, "", req.options)
			.populate(references["posts"]);
		const postsCount = await resourceModel[
			"posts"
...
export const createLike = async (req, res) => {
	try {
		...
		if (!likeCheck) {
			...
			const postsLike = await resourceModel["posts"].findById(
				req.params["id"]
			);
...
export const createComments = async (req, res) => {
	try {
		...
		if (commentUpdate.nModified == 1) {
			...
			const getPost = await resourceModel["posts"].findById(
				req.params["id"]
			);
			return sendRsp(res, 200, "OK", {
				posts: getPost,
...
export const createSubComments = async (req, res) => {
	try {
		...
		const comment = await resourceModel["comments"].findById(
			req.params["comment"]
		);
		if (!comment) {
			return sendRsp(res, 404, "Post not found");
		...
		const updatedComments = await resourceModel["comments"].findById(
			req.params["comment"]
		);
		return sendRsp(res, 200, "OK", {
			posts: post,
